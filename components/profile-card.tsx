...
const occupationTypeColors: Record<Profile["occupationType"], string> = {
  ...
  freelance: "bg-emerald-500/10 text-emerald-500 hover:bg-emerald-500/20",
  employee: "bg-amber-500/10 text-amber-500 hover:bg-amber-500/20",
  entrepreneur: "bg-rose-500/10 text-rose-500 hover:bg-rose-500/20",
}

export function ProfileCard({ profile, onViewMore }: ProfileCardProps) {
  ...
  return (
    <Card className="group relative overflow-hidden transition-all duration-300 hover:shadow-lg hover:shadow-primary/5 hover:-translate-y-1 border-border/50 bg-card/50 backdrop-blur-sm">
      ...
      <CardHeader className="relative flex flex-row items-center gap-4 pb-2">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 mt-1">
            <Badge 
              variant="secondary" 
              className={`${occupationTypeColors[profile.occupationType]} border-0 text-xs font-medium`}
            >
          ...
          <TooltipProvider>
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  ...
                  >
                    ...
                    <span className="sr-only">Copier le téléphone</span>
                  </Button>
                </TooltipTrigger>
                <TooltipContent>
                  <p>{copiedField === "phone" ? "Copié !" : "Copier"}</p>
                </TooltipContent>
              </Tooltip>
            </div>
            ...
            <div className="flex items-center justify-between gap-2 text-muted-foreground">
              ...
              <Tooltip>
                ...
                <TooltipContent>
                  <p>{copiedField === "email" ? "Copié !" : "Copier"}</p>
                </TooltipContent>
              </Tooltip>
            </div>
          </TooltipProvider>
          ...
          <div className="flex items-center gap-3 pt-1">
            {profile.snapchat && (
              <div className="flex items-center gap-1.5 text-muted-foreground hover:text-foreground transition-colors cursor-pointer">
                ...
                <span className="text-xs">@{profile.snapchat}</span>
              </div>
            )}
            {profile.instagram && (
              <div className="flex items-center gap-1.5 text-muted-foreground hover:text-foreground transition-colors cursor-pointer">
